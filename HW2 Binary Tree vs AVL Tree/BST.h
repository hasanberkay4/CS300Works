...
class BinarySearchTree
{
    public:
		...
		void removeSect( const Object & x );
	...
	private:
		...
		void removeSect( const Object & x, BinaryNode<Object> *& t ) const;
...
struct BSTSection
{
	...
	BSTSection(){};
	BSTSection(string t): title(t){};

	friend bool operator > ( const BSTSection & x, const BSTSection & rhs)	{ return rhs.title < x.title; };
	friend bool operator < ( const BSTSection & x, const BSTSection & rhs)	{ return rhs.title > x.title; };
	friend bool operator == ( const BSTSection & x, const BSTSection & rhs)	{ return rhs.title == x.title; };
	void operator = (const BSTSection & rhs)	{ title = rhs.title; items = rhs.items; };

	friend ostream& operator << (ostream & os, const BSTSection & rhs) { os << rhs.title; return os; };
};
